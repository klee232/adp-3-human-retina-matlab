...
% current_series: octa ground truth generated from the previous phase (2D image array)
% current_series_oct: original oct image for this slice (2D image array)
% octa_gt_storage: octa ground turth generated from the previous phase (But
...
function [current_series,sign]=image_groundtruth_refiner_octa_rev(i_slice,current_series,current_series_octa,octa_gt_storage,octa_storage)
    %% setup indicator
    sign="continue";

    %% setup manual selection window for gap filling
    % get the MIP of B-scan
    OCTA_B_scan_x=squeeze(max(octa_gt_storage,[],2));
    OCTA_B_scan_y=squeeze(max(octa_gt_storage,[],3));
    OCTA_B_scan_x_org=squeeze(max(octa_storage,[],2));
    OCTA_B_scan_y_org=squeeze(max(octa_storage,[],3));
    ...
    % octa part
    ax1=subplot(3,4,1);
    img_title=strcat("OCTA groundtruth Slice: ",string(i_slice));
    ...
    % B-scan on x plane
    ax2=subplot(3,4,5);
    img_title=strcat("OCTA B-scan on x plane groundtruth Slice: ",string(i_slice));
    imshow(OCTA_B_scan_x,'Parent',ax2);
    title(img_title);
    % B-scan on y plane
    ax3=subplot(3,4,9);
    img_title=strcat("OCTA B-scan on y plane groundtruth Slice: ",string(i_slice));
    imshow(OCTA_B_scan_y,'Parent',ax3);
    title(img_title);
    ...
    img_title='Cropped Region from OCTA';
    title(img_title);
    ...
    croppedRegion2 = imcrop(current_series_octa, pos_oct);
    ...
    while true
        ...
        OCTA_ROI_pos=hRect.Position;
        OCTA_ROI_pos_y=OCTA_ROI_pos(2);
        OCTA_ROI_pos_x=OCTA_ROI_pos(1);
        OCTA_ROI_pos_h=OCTA_ROI_pos(3);
        OCTA_ROI_pos_w=OCTA_ROI_pos(4);
        % get the current series image size
        img_h=size(current_series,1);
        img_w=size(current_series,2);
        % if this is the first round, launch the brush directly
        if counter == 1
           % launch image binarization brush
           [current_series,croppedRegion1]=image_groundtruth_refiner_octa_brush(h_ax1img,h_ax7img,current_series,croppedRegion1,hRect,ax7);
           ...
           counter=counter+1;
            ...
            if waitforbuttonpress
                ...
                % (subtraction in y)
                if strcmp(cc,'w')
                    update_ROI_pos_y=OCTA_ROI_pos_y-OCTA_ROI_pos_h;
                    % avoid out of boundary circumstance
                    if update_ROI_pos_y<=0
                        update_ROI_pos_y=OCTA_ROI_pos_y;
                    end
                    ...
                    croppedRegion2=imcrop(current_series_octa,hRect_oct.Position);
                ...
                % (subtraction in x)
                elseif strcmp(cc,'a')
                    update_ROI_pos_x=OCTA_ROI_pos_x-OCTA_ROI_pos_w;
                    % avoid out of boundary circumstance
                    if update_ROI_pos_x<=0
                        update_ROI_pos_x=OCTA_ROI_pos_x;
                    end
                    % upade current rectangle ROI position
...
function [current_series,croppedRegion1]=image_groundtruth_refiner_octa_brush(h_ax1img,h_ax3img,current_series,croppedRegion1,h_Rect,hAx_crop1)
    while true
