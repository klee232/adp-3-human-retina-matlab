...
function [network]=coarse_stage_prototype_4_DVC(input)
    ...
    for i_layer=(num_layer):(-1):1
         ...
         network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("skip_dilated_b1c1_",string(i_layer)));
         network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("skip_dilated_b2c1_",string(i_layer)));
         network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("skip_dilated_b3c1_",string(i_layer)));
         network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("skip_cat_",string(i_layer),'/in1'));
         ...
         network=connectLayers(network, strcat("skip_cat_",string(i_layer)),strcat('res_dblock_transcat_cat_',string(i_layer),"/in2"));
         for i_block=1:current_num_block
