...
% This function is built to compute the testing criteria:Kappa
...
function testing_function_pixel_Kappa(trained_coarse_network, trained_fine_network, test_data, test_gt_data, lyr)
    ...
    for i_test_data=1:num_test_data
        ...
        TP = sum(binary_test_out(:) == 1 & test_gt(:) == 1);
        TN = sum(binary_test_out(:) == 0 & test_gt(:) == 0);
        FP = sum(binary_test_out(:) == 1 & test_gt(:) == 0);
        FN = sum(binary_test_out(:) == 0 & test_gt(:) == 1);

        % compute observed aggreement
        po=(TP+TN)/(row_size*col_size);

        % compute expected aggreement
        pe=((TP+FP)*(TP+FN)+(FN+TN)*(FP+TN))/(row_size*col_size)^2;

        % compute Kappa
        current_kappa=(po-pe)/(1-pe+eps);
        total_kappa=total_kappa+current_kappa;
    end


    %% compute accuracy
    kappa=total_kappa/num_test_data;
    msg=strcat("Current layer: ", lyr," , kappa: ", num2str(kappa));
    disp(msg);
