...
% This function is built to compute the testing criteria: geometric-mean
...
function testing_function_pixel_Gmean(trained_coarse_network, trained_fine_network, test_data, test_gt_data, lyr)
    ...
    for i_test_data=1:num_test_data
        ...
        sensitivity = TP / (TP + FN);  % eps to avoid div-by-zero
        specificity = TN / (TN + FP);

        % compute G-mean
        current_gmean=sqrt(sensitivity*specificity);
        total_gmean=total_gmean+current_gmean;
    end


    %% compute accuracy
    gmean=total_gmean/num_test_data;
    msg=strcat("Current layer: ", lyr," , Gmean: ", num2str(gmean));
    disp(msg);
