...
function [network]=coarse_stage_prototype_5_SVC(input)
    ...
    radix=2;
    num_layer=4;
    num_feat=[128 256 512 1024];
    num_block=[2 2 2 2];
    % level 1: 76*76*128, layer 2: 38*38*256, layer 3: 19*19*512, layer 4
    % (bottleneck): 19*19*1024
    for i_layer=1:num_layer
        ...
        for i_block=1:current_num_block
            ...
            if i_block==current_num_block && i_layer~=4 % insert max pooling to all layers (except for layer 5 (bottleneck))
                current_lyr_maxPool=[
    ...
    % decoder part
    % layer 3: (38,38) , layer 2 (76,76), layer 1 (152,152)
    ...
    for i_layer=num_layer:(-1):1
        ...
        for i_block=1:current_num_block
            ...
            if i_block==1
                ...
                res_dblock_dilate=[
                    convolution2dLayer([3 3],current_num_feat,"WeightsInitializer",'he','DilationFactor',[2 2],Name=strcat("res_dblock_dilate_c_",string(i_layer)),Padding="same");
                    ];
                [network]=addLayers(network,res_dblock_transcat);
                [network]=addLayers(network,res_dblock_dilate);
                network=connectLayers(network, strcat("res_dblock_dilate_c_",string(i_layer)),strcat('res_dblock_transcat_cat_',string(i_layer),"/in2"));
                if i_layer==3
                    ...
                    network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("res_dblock_dilate_c_",string(i_layer)));
                else
            ...
            if i_block==1 % checked
                res_dblock_drop=[
                    res_dblock_spatial_dropout_layer(strcat('res_dblock_drop_',string(i_layer)),0.15);
                    ];
                [network]=addLayers(network,res_dblock_drop);
                network=connectLayers(network, strcat('res_dblock_transcat_cat_',string(i_layer)), strcat('res_dblock_drop_',string(i_layer)));
            end
            [network]=res_dblock_bottleneck(network, i_layer, i_block, current_num_feat, radix, num_cardinal);

            if i_block==1
                network=connectLayers(network, strcat('res_dblock_drop_',string(i_layer)),strcat('d_main_layer_c1_',string(i_layer),'_',string(1)));
                network=connectLayers(network, strcat('res_dblock_drop_',string(i_layer)), strcat('d_residual_layer_c1_',string(i_layer), "_", string(1)));
            else
    ...
    res_dblock_transcat=[
         transposedConv2dLayer([2 2],current_num_feat,"Stride",[2 2],Name='res_dblock_transcat_t_final');
         res_block_pad_cat_layer('res_dblock_transcat_cat_final',2);
    ];
    [network]=addLayers(network,res_dblock_transcat);
    network=connectLayers(network, strcat('d_drop_layer_a1_',string(i_layer), "_", string(num_block(i_layer))),'res_dblock_transcat_t_final');
    network=connectLayers(network, 'input_lyr_r3',strcat('res_dblock_transcat_cat_final',"/in2"));
    % decoder block
    num_feat=64;
    final_decoder=[
        convolution2dLayer([3 3],num_feat,Stride=1,Padding=[1 1],Name='final_decoder_c1');
        batchNormalizationLayer(Name='final_decoder_b1');
        reluLayer(Name='final_decoder_r1'); % feature map size 304*304*64
        convolution2dLayer([3 3],num_feat,Stride=1,Padding=[1 1],Name='final_decoder_c2');
        batchNormalizationLayer(Name='final_decoder_b2');
        reluLayer(Name='final_decoder_r2'); % feature map size 304*304*64
        convolution2dLayer([3 3],num_feat,Stride=1,Padding=[1 1],Name='final_decoder_c3');
        batchNormalizationLayer(Name='final_decoder_b3');
        reluLayer(Name='final_decoder_r3'); % feature map size 304*304*64
        ];
    network=addLayers(network,final_decoder);
    network=connectLayers(network, 'res_dblock_transcat_cat_final','final_decoder_c1');
    ...
    network=connectLayers(network, 'final_decoder_r3','layer_flatten_coarse_p_c1');


    %% connect coarse stage (c branch)
    % level 2: (76,76), level 1: (152,152)
    % decoder part
    for i_layer=1:1     
         current_num_feat=num_feat(i_layer);
         current_num_block=num_block(i_layer);
         ...
         res_dblock_d_dilate=[
             convolution2dLayer([3 3],current_num_feat,"WeightsInitializer",'he','DilationFactor',[2 2],Name=strcat("res_dblock_d_dilate_c_",string(i_layer)),Padding="same");
         ];
         res_dblock_d_cat=[
             res_block_pad_cat_layer(strcat('res_dblock_d_transcat_cat_',string(i_layer)),2);
             res_dblock_spatial_dropout_layer(strcat('res_dblock_d_drop_',string(i_layer)),0.15);
             ];
         ...
         [network]=addLayers(network,res_dblock_d_dilate);
         network=connectLayers(network, strcat('res_block_final_add_',string(i_layer),"_",string(num_block(i_layer))),strcat("res_dblock_d_dilate_c_",string(i_layer)));
         [network]=addLayers(network,res_dblock_d_cat);
         network=connectLayers(network, strcat('res_dblock_d_transcat_t_',string(i_layer)),strcat('res_dblock_d_transcat_cat_',string(i_layer),"/in1"));
         network=connectLayers(network, strcat("res_dblock_d_dilate_c_",string(i_layer)),strcat('res_dblock_d_transcat_cat_',string(i_layer),"/in2"));
         for i_block=1:current_num_block
    ...
    % add final decoder for c branch
    % transpose and concatenation block
    res_dblock_d_transcat=[
    ...
    network=connectLayers(network, 'input_lyr_r3',strcat('res_dblock_d_transcat_cat_final',"/in2"));
    % decoder block
    num_feat=64;
    final_decoder=[
