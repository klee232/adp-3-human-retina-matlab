...
function [network]=rose_1_model_DVC_launcher(input)
    ...
    for i_layer=1:num_layer
        ...
        for i_block=1:current_num_block
            ...
            if i_block==current_num_block && i_layer~=1
                current_lyr_maxPool=[
    ...
    for i_layer=num_layer:(-1):1
        ...
        % if this is the deepest layer, don't do any feature concatenation
    ...
    layer_flatten_coarse_p=[
          transposedConv2dLayer([2 2],current_num_feat,"Stride",[2 2],Name=strcat('layer_flatten_coarse_p_t1'));
    ...
    network=connectLayers(network, strcat('d_drop_layer_a1_',string(i_layer), "_", string(1)),'layer_flatten_coarse_p_t1');


    %% connect coarse stage (c branch)
    % decoder part
    for i_layer=(num_layer-4):(-1):1
         current_num_feat=num_feat(i_layer);
         ...
         [network]=addLayers(network,res_dblock_transcat);
         network=connectLayers(network, 'input_lyr_r3',strcat('res_dblock_d_transcat_t_',string(i_layer)));
    ...
    fine_block=[
        ...
        convolution2dLayer([3 3],num_feat,Padding="same",Name='fine_block_c3',WeightsInitializer="narrow-normal", BiasInitializer=@(sz) single(cat(3,zeros(1,1,floor(num_feat/2)), 1, zeros(1,1,(sz(3)-floor(num_feat/2)-1)))));
    ...
    %% conduct c branch propagation
     %% connect fine stage (propagation coefficients stage)
    fine_block_c=[
        fine_block_propagation_layer(2,5,'fine_block_c');
    ];
    network=addLayers(network,fine_block_c);
    network=connectLayers(network, 'layer_flatten_coarse_c_a1','fine_block_c/in1');
    network=connectLayers(network, 'fine_block_c_a1','fine_block_c/in2');
