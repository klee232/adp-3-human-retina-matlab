...
function [p2,binary_choroid_image]=image_groundtruth_generator_binary_hist_chor(p2,choroid_image)
   ...
   %% conduct binarization for  choroid layer image
    ...
    for i_choroid_slice=1:num_choroid_slice
        ...
        [counts,~]=imhist(current_choroid_slice);
        threshold=otsuthresh(counts);
        binary_choroid_image(i_choroid_slice,:,:)=imbinarize(current_choroid_slice,threshold);
    end
