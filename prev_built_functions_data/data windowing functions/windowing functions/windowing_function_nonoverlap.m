...
function [wind_img_train,wind_gt_train]=windowing_function_nonoverlap(img_train,gt_train)
    % retrieve the size information
    [row_img,col_img,num_img]=size(img_train);
    % start partitioning
    wind_img_train=zeros(row_img/8,col_img/8,64,num_img,'uint8');
    wind_gt_train=zeros(row_img/8,col_img/8,64,num_img,'uint8');
    for i_img=1:num_img
        pointer=1;
        for i_row=1:row_img/8:row_img
            for i_col=1:col_img/8:col_img
