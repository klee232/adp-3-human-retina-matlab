...
% Input parameter:
% current_oct_img_gt: original oct ground truth
% picture_obj: struct including image information

% Output:
% filtered_oct_img_gt: processed oct ground truth
% picture_obj: processed picture_obj


function [filtered_oct_img_gt, picture_obj]=image_groundtruth_refiner_oct(picture_obj,current_oct_img_gt)
    ...
    if ~isfolder(dataset_dir)
        ...
        if isfile("temp_data/oct_data_gt_series.mat") && isfile("temp_data/oct_data_gt_ind.mat")
            % if there exists a temp_data folder, load the file inside
            load("temp_data/oct_data_gt_series.mat");
            load("temp_data/oct_data_gt_ind.mat");
            % grab out only the region that contains content based on the previous
            % manually selected region
            z_ind=picture_obj.z_ind;
            start_ind=i_slice;
    ...
    for i_slice=start_ind:end_ind
        disp(string(i_slice))
        current_series=squeeze(filtered_oct_img_gt(i_slice,:,:));
