...
function network=fully_connected_encoder_decoder_net_launcher(input,img_row, img_col, num_features, num_classes)

    %% setup layers in the network
    % setup layers
    patch_size=floor(img_row/2);
    ss_mlp_patch_former_layer=patch_former_layer('ss_mlp_patch_former_lyr',patch_size);
    ss_mlp_patch_embedded_layer=patch_embedded_layer('ss_mlp_patch_embedded_lyr',input);
    ss_mlp_transpose_layer=patch_transpose_layer('ss_mlp_transpose_lyr');
